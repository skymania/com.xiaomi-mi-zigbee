...
class ZigBeeDevice extends MeshDevice {
	 ...
	 * @param {number} [endpointId=0] - The endpoint index (e.g. 0)
	 * @param {boolean} [force=false] - If true, registering will not fail on missing attributes and/or clusters. Warning this might fail, use only if certain that the device accepts this.
	 * @returns {Promise} Resolves if configuration succeeded
	 */
	async registerAttrReportListener(clusterId, attrId, minInt, maxInt, repChange = null, triggerFn, endpointId = 0, force = false) {
		...
		if (!this.node.endpoints[endpointId] || (!force && !this.node.endpoints[endpointId].clusters[clusterId])) {
